...
> 更新日期：`2026-07-04`
>
> 适用固件：`duck-mid-f103 0.3.0`

## 修订记录

| 版本 | 日期 | 说明 |
| --- | --- | --- |
| `v0.3.0` | `2026-07-04` | 新增云台 RPY 目标、RK 姿态反馈和目标姿态组合命令；固件版本响应更新为 `duck-mid-f103 0.3.0` |
...
当前 UART 线协议帧版本为 `0x01`。本次文档和业务版本升级为 `v0.3.0`，但帧格式没有变化，所以帧头中的 `version` 字段仍保持 `0x01`。
...
| 字节 | 类型 | 含义 |
| --- | --- | --- |
| Byte0~Byte3 | `int32` | Roll 目标角度，单位：mrad |
| Byte4~Byte7 | `int32` | Pitch 目标角度，单位：mrad |
| Byte8~Byte11 | `int32` | Yaw 目标角度，单位：mrad |

Roll 和 Pitch 的目标值使用姿态坐标系；Yaw 的目标值使用 yaw 电机角度坐标系。
...
| 字节 | 类型 | 含义 |
| --- | --- | --- |
| Byte0~Byte3 | `int32` | 当前 Roll 姿态角，单位：mrad |
| Byte4~Byte7 | `int32` | 当前 Pitch 姿态角，单位：mrad |
| Byte8~Byte11 | `int32` | 当前 Yaw 姿态角，单位：mrad |

当前控制策略中，Roll 和 Pitch 位置环使用该姿态反馈；Yaw 位置环使用电机主动上报的 yaw 电机角度。Yaw 姿态字段会被保存，但当前不参与 Yaw 位置环。

### 5.5 设置云台 RPY 目标和 RK 当前姿态反馈
...
| Byte4~Byte7 | `int32` | Pitch 目标角度，单位：mrad |
| Byte8~Byte11 | `int32` | Yaw 目标角度，单位：mrad |
| Byte12~Byte15 | `int32` | 当前 Roll 姿态角，单位：mrad |
| Byte16~Byte19 | `int32` | 当前 Pitch 姿态角，单位：mrad |
| Byte20~Byte23 | `int32` | 当前 Yaw 姿态角，单位：mrad |

该命令用于 RK 在同一周期内同时刷新控制目标和姿态反馈，减少目标与反馈不同步的窗口。
